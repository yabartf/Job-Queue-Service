...
Ordering in SQL keeps the scale continuous, makes the tie-break explicit, and leaves room for aging without a redesign — though not, as I first wrote here, for free: see §5.
...
- **No aging, so low priority can starve — and I had the cost of the fix wrong.** A sustained stream of high-priority work means priority-0 jobs never run. Two earlier drafts of this document called aging "a one-line change to the `ORDER BY`", which is true of the diff and false of the consequence. A sort key containing `now()` is not a stored value, so no index can return it in order: `EXPLAIN` against a real database turns `Index Scan using ix_jobs_claim` into a `Sort` over every eligible row, on the most frequent query in the system, exactly under the large backlog where dequeue efficiency is being measured. `W2-17` fails first, because it asserts the plan. The three approaches that do keep the index — a promotion sweep, a lottery claim every Nth time, or a stored effective priority recomputed by the maintenance pass — are compared in `specs/04-claiming.md` §4. Each pays somewhere else, and choosing between them needs a decision about how fast a job should age that is a product question I had no basis to answer. Starvation is at least *visible*: `oldest_pending_seconds` climbing while throughput stays healthy is exactly its signature.
- **Unbounded table growth — the one item here that needs real design work, not just time.** The partial indexes keep the hot paths fast as terminal rows accumulate, but nothing removes them, so the table, its `job_logs` children and every idempotency key grow without bound. A real deployment needs a retention policy. Keeping keys forever is a deliberate choice and not an accident — the requirement is a floor of 24 hours, and expiring a key would mean a client retrying a submission later gets a second job instead of its original, which is a worse failure than a large table. But a policy is still needed, and it is genuinely constrained: it inherits a 24-hour floor that must be enforced in configuration rather than documented, it makes an unreachable branch in `JobService.submit` reachable (a key deleted between the insert's conflict and the read that follows it), and deleting a job deletes the audit trail that answers why it ended as it did. Those constraints are written down in `specs/01-data-model.md` §8 so the work starts from them rather than rediscovering them.
- **No backpressure, and no rate limiting — two different absences, and the interesting one is not the obvious one.** Submission is unbounded: a client can enqueue a million jobs and nothing pushes back. Rate limiting is the reflex answer and it is the weaker of the two here. It caps requests per second; it does nothing about a client that accumulates a million jobs slowly, which is the failure mode a *queue* actually has. The mechanism that fits is a depth check — refuse to submit while `pending` is above a ceiling — and that is roughly ten lines using the count `/health` already computes, with one real cost: a `COUNT` on the hot submit path, so it needs an approximate counter or a threshold read on an interval rather than per request.

  Rate limiting proper was left out for two reasons beyond it not being asked for. First, **there is no principal to limit.** With no authentication the only key available is the client address, and behind a reverse proxy that is the proxy's address unless `X-Forwarded-For` is trusted — which, without a configured trusted-proxy hop count, is a limit anyone bypasses with one header. A limiter that looks like protection and is not is worse than none. Second, **the shared-state version collides with an invariant this design rests on.** Per-process buckets are cheap — the seam exists, `BodySizeLimitMiddleware` already rejects before parsing and returns the standard error envelope — but with N API replicas the effective ceiling is N times the intended one. Making it correct means shared state in Redis, and Redis in this system is deliberately non-authoritative and never raises. A limiter has no fallback to degrade to: fail-open means it stops limiting during precisely the incident it exists for, and fail-closed turns a cache outage into a full outage, contradicting §7's decision that Redis being down is not a `503`. That choice is the design work, and it is not a middleware.

  In production this belongs at the edge — `limit_req` in nginx, or the gateway — where one component does it once instead of every replica reimplementing it.
...
**An audit trail nobody can read is not observability.** `job_logs` was written on every transition from part 1 onward, by the API and the worker alike, and for two parts of the project the only way to read it was `psql`. That is a gap of the kind that is easy not to see, because nothing fails: the data is correct, the tests pass, and the capability is simply unreachable. `GET /jobs/{id}/logs` closes it. The job row carries only the *latest* error, so without the history a job that failed twice and then succeeded presents as an ordinary success, and one the reaper recovered three times presents as a job on attempt four with no account of the other three.

It is a sub-resource rather than a field on `GET /jobs/{id}` because history is unbounded while that endpoint is polled in a loop — embedding it would make the most-fetched response in the system grow with every retry, for callers that almost always only want `status`. Oldest-first, unlike the job list, because this is a timeline read start to finish rather than a feed; and the tie-break is the sequence, not the timestamp, since rows written in one transaction share `now()`.
...
**A warning that fires constantly is worse than no warning.** That principle was violated in the first version of the Redis dispatch and the log stream said so plainly. `next_hint` blocks on `BZPOPMIN` for the poll interval, and redis-py's default socket read timeout is 5 seconds — the same value. The socket always won the race, so every idle poll raised instead of returning empty, and each one logged `dispatch.next_hint_failed`: the identical line emitted when Redis is genuinely unreachable, twice every five seconds, against a healthy server. Nothing broke, because the fallback claim carries the work — which is exactly why an entirely green suite and a Docker smoke test never noticed. The client's read budget is now derived from the longest block it will carry rather than inherited from a library default, and the relationship is pinned by a test instead of by two numbers that happened to be equal.

**The same warning, counted rather than read, found a second bug.** Running the manual plan against the live stack showed 46 of those lines in two minutes — correct, because that pass had stopped Redis on purpose. What was not correct was the *rate*. `next_hint` is the only pacing the slot loop has: `run_forever` has no sleep of its own in the idle path, which is precisely why `NullDispatch.next_hint` sleeps for the timeout it is given, and says so in a comment. `RedisDispatch.next_hint` did not — a refused connection fails in a round trip, so the failure branch returned `None` at once and the slot went straight back round. A controlled twenty-second outage measured **49 idle cycles where the poll interval intends 8**, each one firing a claim query at PostgreSQL: the database an operator most wants left alone while the other one is down. Degrading gracefully is not the same as returning early, and the fix waits out the remainder of the block — the remainder, because a read that timed out has already spent it and sleeping the full timeout again would double the idle interval. Re-measured on the running stack afterwards: 10.
